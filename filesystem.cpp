...
char* HeadSig = (char*)"AymFileSystem\x01\x02";
...
int fs_open(const char* path) {
	...
	file_node_t* tmpbuff = (file_node_t*)malloc(node_size * 4444);
	DWORD tmpsize = node_size * 4444;
	DWORD tmp = ZSTD_decompress(tmpbuff, tmpsize, f_node, node_size);
	node_size = tmp;
	f_node = tmpbuff;
	return FS_SUCCESS;
}
	...
	if (f_wrtnode) {
		...
		for (i = 0; i < f_header.f_count; i++) {
			...
			if (f_comprbuf && f_comprlength) {
				...
				unsigned long HashName = gen_toc_seed(to_string(MurmurHash64B(f_wrtnode[i].f_name,strlen(f_wrtnode[i].f_name), 0xEE6B27EB)).c_str());
		...
		char* disname = (char*)malloc((sizeof(file_node_t) * f_header.f_count));
		DWORD dissize = (sizeof(file_node_t) * f_header.f_count);
		DWORD dist=ZSTD_compress(disname, dissize, f_builder, (sizeof(file_node_t) * f_header.f_count), 9);

		//fwrite(f_builder, (sizeof(file_node_t) * f_header.f_count), 1, fd);
		fwrite(disname, dist, 1, fd);
		fflush(fd);
	}
}
void fs_close() {
...
void fs_readfile(const char* f_name, void** data, unsigned long* f_size) {
	...
	unsigned long HashName = gen_toc_seed(to_string(MurmurHash64B(f_name, strlen(f_name), 0xEE6B27EB)).c_str());
	for (i = 0; i < f_header.f_count; i++) {
		//if (_stricmp(f_node[i].f_name, f_name) == 0) {
